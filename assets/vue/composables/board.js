...
function putPiecesOnBoard() {
    ...
    for (const color of colors) {
        ...
        for (let i = 1; i <= boardDimension; ++i) {
            ...
            pieces.push({ image: `images/pawn_${color}.png`, x: i, y: pawnY, type: 'Pawn' });
        }

        let left = 1, right = 8;
        for (let specialPiece of specialPieces) {
            pieces.push({ image: `images/${specialPiece.toLowerCase()}_${color}.png`, x: left++, y: y, type: specialPiece});
            pieces.push({ image: `images/${specialPiece.toLowerCase()}_${color}.png`, x: right--, y: y, type: specialPiece});
        }

        /* KING AND QUEEN */
        pieces.push({ image: `images/king_${color}.png`, x: 5, y: y, type: 'King' });
        pieces.push({ image: `images/queen_${color}.png`, x: 4, y: y, type: 'Queen' });
    }
